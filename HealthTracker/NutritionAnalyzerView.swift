...
//  HealthTracker
//
//  Created by Swarasai Mulagari on 12/07/24.
//
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                Rectangle()
                    .fill(scoreColor)
                    .frame(width: CGFloat(score) / 6 * geometry.size.width, height: 20)
                    .cornerRadius(10)
            }
        }
        .frame(height: 20)
...
struct ScannerView: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, VNDocumentCameraViewControllerDelegate {
        ...
        func generateAnalysis(fat: Int, cholesterol: Int, sodium: Int, carbs: Int, protein: Int) -> NutritionAnalysis {
            var details: [String] = []
            var healthScore = 0
            ...
            if healthScore >= 4 {
                ...
                overallAssessment = "Overall, this food item may not be the healthiest choice due to its [list top 2-3 negative aspects]. Consider it as an occasional treat rather than a regular part of your diet. For a healthier diet, look for foods with lower [mention high aspects] and higher [mention low aspects]."
            }
